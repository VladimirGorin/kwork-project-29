...
class Parsing():
    ...
    def getCartProfile(self, link, CurrentFile):
        browser = self.browser 
        print("HERE")
    ...
    #""" Overview info """
            ...
            if(searchTitleChecker == True):
                ...
                for element_ in browser.find_elements(By.XPATH, "//div[contains(@class, 'product-title')]//child::*"):
                    if element_.tag_name == "h1":
                        ProductDetailArray.append(element_.text)   
...
for key, page in catalog_pages.items():
    ...
    with open(f"./info/Links{CurrentFile}.json", "r") as file:
        ...
        for link, page in data.items():
                
            print(CurrentFile ,link)
            
            try:
                my_bot.getCurrentPage(link=link)
                my_bot.getCartProfile(link=link, CurrentFile=CurrentFile)
